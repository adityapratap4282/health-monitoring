...
PreparedStatement ps=con.prepareStatement("select * from BMI_Values where user_id=? order by localdate desc,time desc");
ps.setString(1, a);
...
while(rs.next())
{
    ...
    <td class="right"><%=rs.getFloat(2)%></td>
    <td class="right"><%=rs.getFloat(3)%></td>
        <td class="right"><%=rs.getString(5)%></td>
        <td class="right"><%=rs.getDate(6)%></td>
       <td colspan="3" class="right"><input type="button" value=<%=rs.getFloat(4)%> onClick="pay()"></td>
     </tr>
   <%    }  
        }catch (Exception e2) {

		...
		try {
			...
			if (rs.next()) {
				if (rs.getString(10).equals(id) && rs.getString(11).equals(ques) && rs.getString(12).equals(ans)) {
					...
					RequestDispatcher rd = request.getRequestDispatcher("NewPasswordUser.jsp");
					rd.forward(request, response);

				} else {
					RequestDispatcher rd2 = request
							.getRequestDispatcher("ForgotUser.jsp?errMsg=INVALID CREDENTIALS...!!!!!!!!!!!");
					rd2.forward(request, response);

				}
			} else {
				RequestDispatcher rd2 = request
						.getRequestDispatcher("ForgotUser.jsp?errMsg=USER DOESN'T EXIST...!!!!!!!!!!!");
				rd2.forward(request, response);

...
public class Service {
	public static String getUserId() {
		...
		try {
			...
			String query = "select cast (substr(max(user_id),3) as number) from user_details";
	...
	public static void addUser(UserDetails u) {
		try {
			Connection con = OracleDatabase.connect();
			...
			String query1 = "insert into user_details values(?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public static UserDetails checkUser(String uid, String pwd) throws Exception {
		UserDetails ud = null;
		...
		if (rs.next()) {
			ud = new UserDetails(rs.getString(10), rs.getString(1), rs.getString(2), rs.getInt(3), rs.getLong(4),
